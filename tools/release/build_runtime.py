...
SPEC_PATH = RELEASE_DIR / "runtime-spec.json"
ALLOWLIST_PATH = RELEASE_DIR / "app-files.txt"
...
def _copy_application(destination: Path) -> None:
    for relative in verify_application_inputs():
        # The repository uses a src/ layout while the immutable runtime places
        # the importable package directly at app/dola_gateway.
        target_relative = Path(*relative.parts[1:]) if relative.parts[0] == "src" else relative
...
def _write_manifest(stage: Path, target: str, details: dict[str, Any]) -> dict[str, Any]:
    manifest = {
        ...
        "app": {
            "directory": "app",
            "entrypoint": "dola_gateway.server:app",
